...
for sender, message in st.session_state.chat_history:
    if sender == "🧑‍💻 You":
        st.markdown(
            f"""
            <div style='text-align: right; color: black; background-color: #dcf8c6; padding: 8px; border-radius: 10px; margin: 5px 0;'>
                <b>{sender}:</b> {message}
            </div>
        ...
        st.markdown(
            f"""
            <div style='text-align: left; color: black; background-color: #f1f0f0; padding: 8px; border-radius: 10px; margin: 5px 0;'>
                <b>{sender}:</b> {message}
            </div>
            """, unsafe_allow_html=True)
